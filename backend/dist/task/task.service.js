...
let TaskService = class TaskService {
    constructor(prisma, activityLogService, notificationService, plannerService) {
        this.prisma = prisma;
    ...
    buildTaskPayload(dto) {
        const nextStatus = dto.status;
        const completedAt = nextStatus ? (nextStatus === client_1.TaskStatus.DONE ? new Date() : null) : undefined;
        const startedAt = nextStatus === client_1.TaskStatus.IN_PROGRESS && !('startedAt' in dto) ? new Date() : undefined;
        return {
    ...
    async validateProjectMember(projectId, userId) {
        if (!userId) {
            return;
        }
        ...
        if (!member) {
            throw new common_1.ForbiddenException('Selected user must be a project member');
        }
    }
    ...
    async getProjectLabels(projectId, userId, isAdmin) {
        await this.checkProjectAccess(projectId, userId, isAdmin);
    ...
    async createLabel(projectId, userId, isAdmin, dto) {
    ...
    async addComment(taskId, userId, dto, isAdmin) {
        const task = await this.prisma.task.findUnique({
            where: { id: taskId },
        ...
        if (!hasAccess) {
            throw new common_1.ForbiddenException('No access to this task');
        }
        ...
        const recipients = new Set([task.assigneeId, task.createdById, task.reviewerId].filter((id) => Boolean(id && id !== userId)));
        for (const recipient of recipients) {
            await this.notificationService.createNotification({
                userId: recipient,
                type: client_1.NotificationType.TASK_COMMENT,
    ...
    async getSubtasks(taskId, userId, isAdmin) {
        const task = await this.prisma.task.findUnique({
            where: { id: taskId },
            include: { project: { include: { members: true } } },
        });
    ...
    async createSubtask(taskId, userId, dto, isAdmin) {
        const task = await this.prisma.task.findUnique({
            where: { id: taskId },
            include: { project: { include: { members: true } } },
        });
    ...
    async toggleSubtask(taskId, subtaskId, userId, completed, isAdmin) {
        const task = await this.prisma.task.findUnique({
            where: { id: taskId },
            include: { project: { include: { members: true } } },
        });
    ...
    async logTime(taskId, userId, dto, isAdmin) {
        const task = await this.prisma.task.findUnique({
            where: { id: taskId },
            include: { project: { include: { members: true } } },
        });
    ...
    async attachLabel(taskId, labelId, userId, isAdmin) {
        const task = await this.prisma.task.findUnique({
            where: { id: taskId },
            include: { project: { include: { members: true } } },
        });
    ...
    async detachLabel(taskId, labelId, userId, isAdmin) {
        const task = await this.prisma.task.findUnique({
            where: { id: taskId },
            include: { project: { include: { members: true } } },
        });
    ...
    async getTaskComments(taskId, userId, isAdmin) {
        const task = await this.prisma.task.findUnique({
            where: { id: taskId },
            include: { project: { include: { members: true } } },
        });
